...
import SpringMechanism.RouteMechanism.ControllerMethod;
import SpringMechanism.RouteMechanism.ControllerSingleton;
import SpringMechanism.RouteMechanism.HttpMethod;
import SpringMechanism.RouteMechanism.RouteMethod;
...
public class ServerThread implements Runnable{
    ...
    public void run(){
        try {
            ...
            String route = request.getLocation();
            HttpMethod httpMethod = request.getMethod().equals(Method.GET) ? HttpMethod.GET : HttpMethod.POST;
            RouteMethod routeKey = new RouteMethod(route,httpMethod);
            ControllerMethod controllerMethod = ControllerSingleton.routeMappings.get(routeKey);

            if (controllerMethod != null) {
                try {
                    controllerMethod.getMethod().invoke(controllerMethod.getControllerInstance());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
